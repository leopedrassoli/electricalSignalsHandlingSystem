# -*- coding: utf-8 -*-
"""
Created on Thu Aug  2 11:26:05 2018
...
icone = path_default + '\\imagens\\icone.png'
fundo = path_default + "\\imagens\\fundo.png"
logos = path_default + "\\imagens\\logos.png"
...
#        self.lb_image.setPixmap(pixmap)
    ...
    def Abrir(self):
        ...
        try:
            ...
            try:
                corrente_list, tensao_list, potencia_list, temperatura_list, combined_list = self.Values_From_Txt(content,'L')
            ...
            self.labelInicio.setText("Início: " + primeiro.strftime('%d/%m/%y - %H:%M:%S'))
...
#            self.dt_Final.setMaximumDateTime(ultimo)
    ...
    def fazerCalculos(self):
        ...
        if self.rb_Periodo.isChecked():
            ...
            corrente_p, tensao_p, potencia_p, temperatura_p, combined_p = self.Values_Periodo(ini,fin,corrente_list,tensao_list,
                                                                               potencia_list, temperatura_list, combined_list,'P')
            ...
            corrente_list, tensao_list, potencia_list, temperatura_list, combined_list = self.Values_From_Txt(content,'P')

        plotCorrente = {"y": corrente_list, "titulo" : "Corrente x Tempo", "xlabel"  :"Tempo", "ylabel" : "Corrente [A]", "color" : "blue"}
        plotTensao = {"y": tensao_list, "titulo" : "Tensão x Tempo", "xlabel"  :"Tempo", "ylabel" : "Tensão [V]", "color" : "red"}
        plotPotencia = {"y": potencia_list, "titulo" : "Potência x Tempo", "xlabel"  :"Tempo", "ylabel" : "Potência [W]", "color" : "green"}
        plotTemperatura = {"y": temperatura_list, "titulo" : "Temperatura x Tempo", "xlabel"  :"Tempo", "ylabel" : "Temperatura [ºC]", "color" : "purple"}
    ...
    def periodoOk(self):
        ...
        if self.rb_Periodo.isChecked():
            ...
            if (dados_dtInicio >= dados_dtFinal) or ((dados_dtFinal-dados_dtInicio) < dt.timedelta(minutes = 1)) or (dados_dtInicio < primeiro) or (dados_dtFinal > ultimo):
                w = QWidget()
                w.setWindowIcon(QtGui.QIcon(QtGui.QPixmap(icone)))
                QMessageBox.information(w, "Atenção", "Verificar período especificado.\nObs: Mínimo de 1 minuto.")
                return False
        return True
    ...
    def Relatorio(self):
        try:
            if self.periodoOk():
                ...
                gr.gerarRelatorio(corrente_list, tensao_list, potencia_list, temperatura_list, combined_list)
                vtg.aux = 1
        except Exception as error:
            print(error)
            w = QWidget()
    ...
    def Grafico(self):
        ...
        if self.periodoOk():
            ...
            if not cbC and not cbT and not cbP and not cbTp:
                QMessageBox.information(w, "Atenção", "Selecione ao menos um item para visualizar!")
            #4 selecionados para o mesmo gráfico
            elif cbC and cbT and cbP and cbTp and self.rb_Juntos.isChecked():
                QMessageBox.information(w, "Atenção", "Máximo de 3 variáveis no mesmo gráfico!")
            else:
                try:
                    self.fazerCalculos()
                    ...
                    cjTp = {"cb": cbTp, "valores": plotTemperatura, 'nome': "Temperatura"}
                    ...
                    ##2 selecionados
                    if total == 2:
                        tit = conjunto[0]['nome'] + ' e ' + conjunto[1]['nome'] + ' x Tempo'
    ...
    def Values_From_Txt(self,content,qual):
        try:
            ...
            for linha in range(len(content)):
                hora, minuto, segundo, mes, dia, ano, corrente, tensao, temperatura = content[linha].split(",")
                dia_list.append(dia)
                mes_list.append(mes)
                ano_list.append(ano)
                ...
                combined_list.append(dt.datetime(int(ano),int(mes),int(dia),int(hora),int(minuto),int(segundo)))

                self.valorBarra(linha,content,qual)

            return corrente_list, tensao_list, potencia_list, temperatura_list, combined_list
        except Exception as error:
                w = QWidget()
                w.setWindowIcon(QtGui.QIcon(QtGui.QPixmap(icone)))
                QMessageBox.information(w, "Atenção", "Erro com a leitura do arquivo!")

    def Values_Periodo(self,inicio, final, corrente_list, tensao_list, potencia_list, temperatura_list, combined_list,qual):
